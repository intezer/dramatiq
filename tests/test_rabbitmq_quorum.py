"""Internal quorum tests -- **not** part of the upstream contribution.

These exercise the internal quorum subclass (see ``tests/quorum_broker.py``)
against a real RabbitMQ 4.3+ server to confirm the base :class:`RabbitmqBroker`
provides everything the subclass needs (overridable queue declaration plus the
``consumer_timeout`` re-lease).
"""
...
from dramatiq.brokers.rabbitmq import DEAD_MESSAGE_TTL, MIN_CONSUMER_TIMEOUT
from dramatiq.common import current_millis
from dramatiq.middleware import Middleware

from .common import RABBITMQ_CREDENTIALS
from .quorum_broker import QuorumRabbitmqBroker


def _rabbitmq_server_version(broker):
    try:
        version = broker.connection._impl.server_properties.get("version", "")
    except Exception:
        return ()
    parts = []
...
def quorum_rabbitmq_broker():
    ...
    try:
        ...
        pytest.skip("No connection to RabbitMQ server.")
...
def test_quorum_broker_inherits_consumer_timeout_lease():
    # The subclass defaults consumer_timeout, and the base class turns that
    # into a delay-queue lease -- proving the re-lease mechanism is available
    # to subclasses for free.
    broker = QuorumRabbitmqBroker(host="127.0.0.1")
    assert broker._consumer_timeout == MIN_CONSUMER_TIMEOUT
...
def test_quorum_broker_enqueues_messages_in_strict_priority_order(quorum_rabbitmq_broker):
    if _rabbitmq_server_version(quorum_rabbitmq_broker) < (4, 3):
